...
const RAW_TASKS = [
  ...
  {id:67,tag:"opt-rec",label:"27.a",isSub:false,step:27,activity:"Vile Vigour ZMI to 95 runecrafting",gp_change:-48000000,gp_total:10323749,chapter:"3.2: Quest cape, quality of life",chapter_num:"3.2",xp_gains:{RC:7561308,CRF:2072000,FLE:8000000,MNG:687391},expected:{ATK:78,STR:88,DEF:76,HP:87,RNG:93,PRY:76,MAG:93,RC:74,CON:75,AGI:98,HRB:84,THI:96,CRF:90,FLE:82,SLY:76,HUN:95,MNG:97,SMI:86,FSH:99,COK:81,FM:89,WC:80,FRM:96,SAI:78},guide:"Optional but recommended: with daeyalt mining and the full Raiments of the Eye, train Runecraft via Vile Vigour fletching + ZMI multiskilling (or non-spellbook-swap with staminas). Getting to 90 or 95 Runecraft sooner gives you ample wrath runes for offering spells during slayer. Blow glass while ZMI crafting, then switch to fletching when out of glass."},
...
const APP_VERSION = "Tracker v4.0";
const GUIDE_UPDATED = "Guide last updated 7 Jun 2026";

/* ================================================================
   XP RATES — EDIT ME
   Effective xp/hr per skill, used to estimate each step's duration
   (a step's time = its biggest xp chunk ÷ that skill's rate, since
   multiskill xp is earned concurrently). Update a number here and
   every estimate on the page recalculates.
   ================================================================ */
const XP_RATES = {
  ATK: 120000,   // slayer melee
  STR: 120000,
  DEF: 120000,
  HP:  1000000,  // passive alongside combat — kept high so it never drives a step
  RNG: 350000,   // chinning
  PRY: 300000,
  MAG: 200000,
  RC:  55000,    // ZMI / lava multiskilling
  CON: 600000,   // mahogany benches
  AGI: 60000,
  HRB: 450000,
  THI: 250000,   // pyramid plunder w/ sceptre
  CRF: 300000,   // superglass + arteglass
  FLE: 260000,   // arteglass fletching (user-verified)
  SLY: 35000,    // slayer xp/hr on-task
  HUN: 250000,   // black chins
  MNG: 85000,    // 3t gems
  SMI: 300000,   // UIM gold
  FSH: 60000,
  COK: 300000,
  FM:  300000,
  WC:  100000,
  FRM: 2000000,  // passive farm runs — high so hourlies don't dominate
  SAI: 60000,
};

/* OSRS hiscores index_lite order */
...
:root{
  --bg:#0a0a0c; --surface:#101013; --raise:#17171c; --inset:#0d0d10;
  --line:#23232c; --line2:#3a3a46;
  --text:#f2f2f4; --dim:#b0b0ba; --faint:#7c7c88;
  --accent:#8b8bff; --accent-deep:#5b5bd6;
  --done:#34d399; --done-dim:#1d7a57;
  --par:#4cc2ff; --par-dim:#1e6a94;
  --later:#b48bff; --later-dim:#6b4aa8;
  --warn:#ff7a72; --warn-dim:#a33c36;
  --optrec:#2dd4bf;
  --mono:'JetBrains Mono',ui-monospace,'SF Mono',Consolas,monospace;
}
html,body{margin:0;padding:0;background:var(--bg);}
*{box-sizing:border-box;}
.app{font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:var(--bg);min-height:100vh;color:var(--text);padding-bottom:80px;font-size:14px;}
.wrap{max-width:1060px;margin:0 auto;padding:0 28px;width:100%;}
.mono{font-family:var(--mono);}
button{font-family:inherit;}
button:focus-visible,input:focus-visible,textarea:focus-visible{outline:2px solid var(--accent);outline-offset:2px;border-radius:6px;}

/* header */
.hdr{position:sticky;top:0;z-index:100;background:rgba(10,10,12,.9);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border-bottom:1px solid var(--line);}
.hdr .bar{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 0 12px;flex-wrap:wrap;}
.wordmark{display:flex;align-items:baseline;gap:10px;}
.wordmark .name{font-weight:800;font-size:17px;letter-spacing:-.03em;}
.wordmark .sub{font-family:var(--mono);font-size:10.5px;color:var(--faint);letter-spacing:.14em;text-transform:uppercase;}
.playerbtn{display:flex;align-items:center;gap:10px;background:none;border:1px solid var(--line);border-radius:10px;padding:6px 12px 6px 7px;cursor:pointer;transition:border-color .15s,background .15s;color:var(--text);}
.playerbtn:hover{border-color:var(--line2);background:var(--surface);}
.avatar{width:26px;height:26px;border-radius:8px;background:linear-gradient(135deg,var(--accent),var(--accent-deep));display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;color:#fff;}
.avatar.empty{background:var(--raise);color:var(--faint);border:1px dashed var(--line2);}
.playerbtn .who{text-align:left;line-height:1.25;}
.playerbtn .rsn{font-weight:600;font-size:13px;}
.playerbtn .meta{font-family:var(--mono);font-size:10.5px;color:var(--dim);}
.playerbtn .meta .bad{color:var(--warn);}
.playerbtn .meta .ok{color:var(--done);}

/* stats + timeline */
.stats{display:flex;align-items:baseline;gap:14px;flex-wrap:wrap;padding-bottom:10px;font-size:13px;color:var(--dim);}
.stats .big{font-size:15px;font-weight:700;color:var(--text);}
.stats .jump{margin-left:auto;background:none;border:none;color:var(--accent);font-size:12.5px;font-weight:600;cursor:pointer;padding:2px 4px;}
.stats .jump:hover{text-decoration:underline;text-underline-offset:3px;}
.tl{display:flex;gap:8px;padding-bottom:14px;}
.tl .grp{display:flex;gap:2px;flex:var(--n) 1 0;}
.tl button{flex:1;height:13px;border-radius:2px;border:none;padding:0;cursor:pointer;background:#40404d;transition:transform .1s,background .15s;min-width:3px;}
.tl button:hover{transform:scaleY(1.5);}
.tl button.done{background:#26a06f;}
.tl button.par{background:#2f8fc4;}
.tl button.later{background:#8a63d2;}
.tl button.opt{background:transparent;box-shadow:inset 0 0 0 1px var(--line2);}
.tl button.opt.done{box-shadow:inset 0 0 0 1px var(--done-dim);background:transparent;}
.tl button.cur{background:var(--accent);animation:pulse 2s ease-in-out infinite;}
...
.drawer .inner{border:1px solid var(--line);border-radius:12px;background:var(--surface);margin-bottom:14px;}
.syncrow{display:flex;gap:8px;padding:14px 16px;border-bottom:1px solid var(--line);flex-wrap:wrap;align-items:center;}
.skills{display:grid;grid-template-columns:repeat(auto-fill,minmax(112px,1fr));gap:1px;background:var(--line);padding:1px;}
.sk{background:var(--surface);padding:9px 11px;display:flex;align-items:center;gap:8px;}
.sk .nm{font-size:11px;color:var(--dim);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.sk .lv{font-family:var(--mono);font-size:13.5px;font-weight:600;}
.sk .req{font-family:var(--mono);font-size:10.5px;color:var(--dim);}
.sk.behind{background:#1a1113;}
.sk.behind .lv{color:var(--warn);}
.sk input{width:36px;background:var(--inset);border:1px solid var(--line2);border-radius:5px;color:var(--text);font-family:var(--mono);font-size:12.5px;font-weight:600;text-align:center;padding:2px 0;outline:none;}
.sk input:focus{border-color:var(--accent);}
.drawer .foot{display:flex;justify-content:space-between;align-items:center;gap:10px;padding:11px 16px;border-top:1px solid var(--line);font-size:12px;color:var(--dim);flex-wrap:wrap;}

/* inputs & controls */
.field{background:var(--inset);border:1px solid var(--line2);border-radius:8px;color:var(--text);font-size:13.5px;padding:8px 12px;outline:none;font-family:inherit;transition:border-color .15s;}
.field:focus{border-color:var(--accent);}
textarea.field{width:100%;resize:vertical;}
.btn{background:var(--accent);border:1px solid var(--accent);color:#0c0c14;font-weight:700;border-radius:8px;padding:8px 16px;font-size:13px;cursor:pointer;transition:filter .12s,transform .08s;}
.btn:hover{filter:brightness(1.1);}
.btn:active{transform:translateY(1px);}
.ghost{background:none;border:1px solid var(--line);color:var(--dim);border-radius:8px;padding:6px 12px;font-size:12.5px;font-weight:600;cursor:pointer;transition:border-color .13s,color .13s;}
.ghost:hover{border-color:var(--line2);color:var(--text);}
.linkish{background:none;border:none;color:var(--dim);font-size:12.5px;cursor:pointer;text-decoration:underline;text-underline-offset:3px;padding:2px 4px;}
.linkish:hover{color:var(--text);}
.seg{display:flex;gap:2px;background:var(--surface);border:1px solid var(--line);border-radius:9px;padding:3px;flex-wrap:wrap;}
.seg button{border:none;background:none;color:var(--dim);padding:4px 11px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:background .12s,color .12s;}
.seg button:hover{color:var(--text);}
.seg button.on{background:var(--raise);color:var(--text);box-shadow:inset 0 0 0 1px var(--line2);}
.seg .n{font-family:var(--mono);font-size:10.5px;color:var(--faint);}
.seg button.on .n{color:var(--dim);}
.searchbox{display:flex;align-items:center;gap:8px;background:var(--surface);border:1px solid var(--line);border-radius:9px;padding:7px 12px;flex:1 1 220px;min-width:0;transition:border-color .15s;}
.searchbox:focus-within{border-color:var(--accent);}
.searchbox input{flex:1;background:transparent;border:none;color:var(--text);font-size:13px;outline:none;min-width:0;font-family:inherit;}
.searchbox .clear{cursor:pointer;color:var(--dim);font-size:13px;border:none;background:none;padding:2px;}
.kbd{font-family:var(--mono);font-size:10px;color:var(--faint);border:1px solid var(--line);border-bottom-width:2px;border-radius:4px;padding:0 5px;}

/* section headers */
.sect{display:flex;align-items:baseline;gap:10px;margin:26px 0 4px;padding-bottom:8px;border-bottom:1px solid var(--line);}
.sect .nm{font-size:13px;font-weight:700;}
.sect .ct{font-family:var(--mono);font-size:11px;color:var(--faint);}
.sect .eta{font-family:var(--mono);font-size:11px;color:var(--dim);margin-left:auto;}

/* rows */
.row{border-bottom:1px solid var(--line);}
.row .line1{display:flex;align-items:center;gap:11px;padding:11px 6px 11px 2px;cursor:pointer;transition:background .1s;border-radius:6px;}
.row .line1:hover{background:rgba(255,255,255,.022);}
.row.cur .line1{box-shadow:inset 3px 0 0 var(--accent);padding-left:10px;}
.row.sub .line1{padding-left:30px;}
.row.sub.cur .line1{padding-left:38px;}
.chk{width:19px;height:19px;border-radius:6px;border:2px solid #6a6a78;background:var(--raise);cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center;color:#0c0c14;font-size:12px;font-weight:800;transition:border-color .12s,background .12s;}
.chk:hover{border-color:var(--done);}
.chk.on{background:var(--done);border-color:var(--done);}
.row.opt-row .chk{border-style:dashed;}
.lbl{font-family:var(--mono);font-size:11px;color:var(--dim);flex-shrink:0;min-width:34px;}
.ttl{flex:1;min-width:0;font-size:14px;font-weight:500;line-height:1.45;}
.row.isdone .ttl{color:#87878f;text-decoration:line-through;}
.row.isdone .lbl{opacity:.5;}
.tag{font-family:var(--mono);font-size:9px;letter-spacing:.08em;padding:2px 6px;border-radius:4px;flex-shrink:0;font-weight:600;}
.tag.rec{color:var(--optrec);box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--optrec) 45%,transparent);}
.tag.opt{color:var(--dim);box-shadow:inset 0 0 0 1px var(--line2);}
.tag.now{color:#0c0c14;background:var(--accent);letter-spacing:.06em;}
.st{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
.st.par{background:var(--par);box-shadow:0 0 7px var(--par);}
.st.later{background:var(--later);box-shadow:0 0 7px var(--later);}
.mchip{font-family:var(--mono);font-size:11px;flex-shrink:0;color:var(--dim);}
.mchip.eta{color:var(--dim);}
.mchip.bd{color:var(--warn);cursor:help;}
.mchip.plus{color:var(--done);}
.mchip.minus{color:var(--warn);opacity:.85;}
.acts{display:flex;gap:2px;flex-shrink:0;opacity:.9;transition:opacity .12s;}
.line1:hover .acts{opacity:1;}
.ib{width:24px;height:24px;border-radius:6px;border:none;background:transparent;cursor:pointer;font-size:12px;color:var(--dim);display:flex;align-items:center;justify-content:center;transition:background .12s,color .12s;}
.ib:hover{background:var(--raise);color:var(--text);}
.ib.on-par{color:var(--par);background:rgba(76,194,255,.12);}
.ib.on-later{color:var(--later);background:rgba(180,139,255,.12);}
.caret{transition:transform .18s;}
.row.open .caret{transform:rotate(90deg);}

/* expandable detail */
.exp{display:grid;grid-template-rows:0fr;transition:grid-template-rows .22s ease;}
.row.open .exp{grid-template-rows:1fr;}
.exp>div{overflow:hidden;min-height:0;}
.detail{padding:4px 2px 18px 32px;max-width:760px;}
.row.sub .detail{padding-left:60px;}
.guide{font-size:13.5px;line-height:1.7;color:#d6d6de;white-space:pre-wrap;}
.dl{font-family:var(--mono);font-size:10px;letter-spacing:.12em;color:var(--dim);text-transform:uppercase;margin:16px 0 7px;}
.dl.bd{color:var(--warn);}
.chips{display:flex;gap:5px 14px;flex-wrap:wrap;font-size:12.5px;}
.chips .c{display:inline-flex;align-items:center;gap:6px;color:var(--dim);}
.chips .c b{color:var(--text);font-weight:600;}
.chips .c.bd b{color:var(--warn);}
.dot{width:7px;height:7px;border-radius:50%;flex-shrink:0;}
.notes{min-height:44px;font-size:13px;margin-top:2px;}

/* hero */
.hero{min-height:82vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 20px;}
.hero .eye{font-family:var(--mono);font-size:11px;letter-spacing:.3em;color:var(--accent);text-transform:uppercase;}
.hero h1{font-size:clamp(30px,5.4vw,52px);font-weight:800;letter-spacing:-.035em;margin:18px 0 14px;line-height:1.08;}
.hero h1 .a{color:var(--accent);}
.hero p{font-size:15px;line-height:1.7;color:var(--dim);max-width:520px;margin:0 0 30px;}
.hero .row{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;}
.hero input{width:min(320px,78vw);padding:11px 14px;font-size:14px;}
.hero .btn{padding:11px 22px;font-size:14px;}

.panel{background:var(--surface);border:1px solid var(--line);border-radius:12px;padding:14px 18px;margin-bottom:14px;}
.hint{display:flex;gap:14px;flex-wrap:wrap;font-size:11px;color:var(--faint);margin:10px 2px 6px;}
.empty{text-align:center;padding:70px 0;color:var(--dim);}
input[type=number]::-webkit-inner-spin-button{opacity:.3;}
::selection{background:rgba(139,139,255,.32);}
*{scrollbar-width:thin;scrollbar-color:#2c2c36 var(--bg);}
::-webkit-scrollbar{width:10px;}::-webkit-scrollbar-thumb{background:#2c2c36;border-radius:5px;border:2px solid var(--bg);}
@media (prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms !important;transition-duration:.01ms !important;}}
@media (max-width:640px){.wrap{padding:0 14px;}.detail{padding-left:24px;}.row.sub .detail{padding-left:40px;}}
`;
...
function estimate(task){
  let hours=0, driver=null;
  for(const [sk,xp] of Object.entries(task.xp_gains||{})){
    const r=XP_RATES[sk]; if(!r||!xp) continue;
    const h=xp/r; if(h>hours){ hours=h; driver=sk; }
  }
  return hours>0?{hours,driver}:null;
}
function fmtH(h){ if(h<1) return "≈"+Math.max(1,Math.round(h*60))+"m"; if(h<10) return "≈"+h.toFixed(1)+"h"; return "≈"+Math.round(h)+"h"; }
...
function loadState(){
  try{ const raw=window.localStorage&&window.localStorage.getItem(STORAGE_KEY);
    if(raw){ const p=JSON.parse(raw);
      const ob = p.onboarded!=null ? !!p.onboarded : !!(p.rsn||Object.keys(p.levels||{}).length||Object.keys(p.done||{}).length);
      return {done:p.done||{},postponed:p.postponed||{},parallel:p.parallel||{},postponedOrder:p.postponedOrder||[],levels:p.levels||{},notes:p.notes||{},rsn:p.rsn||"",onboarded:ob}; }
  }catch(e){}
  return {done:{},postponed:{},parallel:{},postponedOrder:[],levels:{},notes:{},rsn:"",onboarded:false};
}
...
function App(){
  ...
  const setLevel=useCallback((sk,val)=>{ setState(prev=>{ const levels={...prev.levels}; const n=parseInt(val,10); if(!val||isNaN(n)) delete levels[sk]; else levels[sk]=Math.max(1,Math.min(99,n)); return {...prev,levels,onboarded:true}; }); },[]);
    ...
    for(const make of proxies){
      try{
        ...
        if(Object.keys(levels).length>=10){
          setState(prev=>({...prev,levels:{...prev.levels,...levels},rsn:name,onboarded:true}));
          setFetchStatus("✓ Synced "+name+(levels.SAI?"":" — Sailing isn't on the hiscores, add it manually"));
          return;
        }
      }catch(e){}
    }
    setFetchStatus("Couldn't reach the hiscores — check the spelling or try again. Sync works best when this page is hosted online. You can also enter levels manually.");
  },[rsnInput]);
  ...
  const applyImport=useCallback((p)=>{
    ...
    setState({done:p.done||{},postponed:p.postponed||{},parallel:p.parallel||{},postponedOrder:p.postponedOrder||[],levels:p.levels||{},notes:p.notes||{},rsn:p.rsn||"",onboarded:true});
    return true;
  },[]);
  ...
  const expected=(nextUp&&nextUp.expected)||(allTasks[allTasks.length-1]&&allTasks[allTasks.length-1].expected)||{};
  const behindNow=SKILLS.filter(sk=>(expected[sk]||1)>(state.levels[sk]||1));
  const totalLevel=SKILLS.reduce((s,sk)=>s+(state.levels[sk]||1),0);
  const remainH=allTasks.filter(t=>!state.done[t.id]).reduce((s,t)=>{ const e=estimate(t); return s+(e?e.hours:0); },0);

  const chapterMeta={};
  chapters.forEach(ch=>{
    const ct=allTasks.filter(t=>t.chapter===ch);
    const cd=ct.filter(t=>state.done[t.id]).length;
    const eta=ct.filter(t=>!state.done[t.id]).reduce((s,t)=>{ const e=estimate(t); return s+(e?e.hours:0); },0);
    chapterMeta[ch]={done:cd,total:ct.length,eta};
  });
  ...
  const jumpTo=(id)=>{ setExpandedIds(prev=>({...prev,[id]:true})); const el=rowRefs.current[id]; if(el) el.scrollIntoView({behavior:"smooth",block:"center"}); };
  const skipOnboarding=()=>setState(prev=>({...prev,onboarded:true}));
  ...
  if(!state.onboarded){
    return (
      <div className="app">
        <style>{CSS}</style>
        <div className="hero">
          <div className="eye">Bruhsailer · Chapter 3</div>
          <h1>The guide, minus<br/>the <span className="a">spreadsheet</span>.</h1>
          <p>All {total} steps as an interactive checklist — synced to your live stats, with time estimates for every grind and room to reorder things your way.</p>
          <div className="row">
            <input className="field" value={rsnInput} onChange={e=>setRsnInput(e.target.value)}
              onKeyDown={e=>{ if(e.key==='Enter') fetchHiscores(); }} placeholder="Your RuneScape name" aria-label="RuneScape name" autoFocus />
            <button className="btn" onClick={()=>fetchHiscores()}>Sync my stats</button>
          </div>
          {fetchStatus && <div style={{fontSize:12.5, marginTop:16, color:fetchStatus[0]==="✓"?"var(--done)":"var(--dim)", maxWidth:440, lineHeight:1.6}}>{fetchStatus}</div>}
          <div style={{marginTop:20, display:"flex", gap:4, justifyContent:"center", flexWrap:"wrap"}}>
            <button className="linkish" onClick={()=>{ skipOnboarding(); setDrawer(true); setEditing(true); }}>enter levels manually</button>
            <span style={{color:"var(--faint)"}}>·</span>
            <button className="linkish" onClick={skipOnboarding}>skip for now</button>
          </div>
          <div className="mono" style={{position:"absolute", bottom:22, fontSize:10.5, color:"var(--faint)"}}>{APP_VERSION} · {GUIDE_UPDATED}</div>
        </div>
      </div>
    );
  }

  /* ---------- timeline groups (per chapter) ---------- */
  const tlGroups=chapters.map(ch=>allTasks.filter(t=>t.chapter===ch));
        ...
        <div className="wrap">
          <div className="bar">
            <div className="wordmark">
              <span className="name">Bruhsailer</span>
              <span className="sub">Chapter 3 Tracker</span>
            </div>
            <div style={{display:"flex", alignItems:"center", gap:8, flexWrap:"wrap"}}>
              <button className="ghost" onClick={doExport}>Save</button>
              <button className="ghost" onClick={toggleAll}>{allExpanded?"Collapse all":"Expand all"}</button>
              <button className="playerbtn" onClick={()=>setDrawer(d=>!d)} aria-expanded={drawer} title="Skills & sync">
                <span className={"avatar"+(state.rsn?"":" empty")}>{state.rsn?state.rsn[0].toUpperCase():"?"}</span>
                <span className="who">
                  <div className="rsn">{state.rsn||"Set player"}</div>
                  <div className="meta">
                    {hasLevels
                      ? <>total <b>{totalLevel}</b>{behindNow.length>0?<span className="bad"> · ▲{behindNow.length} behind</span>:<span className="ok"> · on track</span>}</>
                      : "sync your stats"}
                  </div>
                </span>
              </button>
              ...
              <div className="inner">
                <div className="syncrow">
                  <input className="field" style={{flex:"1 1 200px", minWidth:0}} value={rsnInput} onChange={e=>setRsnInput(e.target.value)}
                    onKeyDown={e=>{ if(e.key==='Enter') fetchHiscores(); }} placeholder="RuneScape name…" aria-label="RuneScape name" />
                  <button className="btn" style={{padding:"8px 14px"}} onClick={()=>fetchHiscores()}>Sync</button>
                  {state.rsn && <button className="ghost" onClick={()=>fetchHiscores(state.rsn)}>↻ Refresh</button>}
                  <button className="ghost" onClick={()=>setEditing(e=>!e)}>{editing?"Done editing":"✎ Edit manually"}</button>
                  {fetchStatus && <span style={{fontSize:12, color:fetchStatus[0]==="✓"?"var(--done)":"var(--dim)", flexBasis:"100%"}}>{fetchStatus}</span>}
                </div>
                <div className="skills">
                  {SKILLS.map(sk=>{
                    const you=state.levels[sk], exp=expected[sk]||1;
                    const behind=hasLevels&&(you||1)<exp;
                    return (
                      <div key={sk} className={"sk"+(behind?" behind":"")} title={exp>1?`Guide expects ${SKILL_FULL[sk]} ${exp} at your current step`:SKILL_FULL[sk]}>
                        <span className="dot" style={{background:SKILL_COLORS[sk], width:6, height:6}} />
                        <span className="nm">{SKILL_FULL[sk]}</span>
                        {editing
                          ? <input type="number" min={1} max={99} value={you||""} placeholder="1" onChange={e=>setLevel(sk,e.target.value)} aria-label={SKILL_FULL[sk]+" level"} />
                          : <span className="lv">{you||"–"}</span>}
                        <span className="req">{exp>1?"›"+exp:""}</span>
                      </div>
                    );
                  })}
                </div>
                <div className="foot">
                  <span>Levels vs what the guide expects at your current step — <span style={{color:"var(--warn)"}}>red</span> means catch up.</span>
                  <span className="mono" style={{fontSize:10.5}}>{APP_VERSION} · {GUIDE_UPDATED}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="stats">
            <span className="big mono">{doneCount}<span style={{color:"var(--faint)"}}>/{total}</span></span>
            <span>steps done · <span className="mono">{pct}%</span></span>
            {remainH>0 && <span className="mono" title="Estimated active hours left, from the XP rates in this file">{fmtH(remainH)} remaining</span>}
            {nextUpId!=null && <button className="jump" onClick={()=>jumpTo(nextUpId)}>Jump to current step →</button>}
          </div>

          <div className="tl" aria-label="Step timeline">
            {tlGroups.map((grp,gi)=>(
              <div key={gi} className="grp" style={{"--n":grp.length}}>
                {grp.map(t=>{
                  const d=!!state.done[t.id];
                  const cls=[ t.tag&&"opt", d&&"done", !d&&t.id===nextUpId&&"cur", !d&&state.parallel[t.id]&&"par", !d&&state.postponed[t.id]&&"later" ].filter(Boolean).join(" ");
                  return <button key={t.id} className={cls} title={t.label+" · "+t.activity} onClick={()=>jumpTo(t.id)} aria-label={"Go to step "+t.label} />;
                })}
              </div>
            ))}
          </div>
      ...
      <div className="wrap">
        {showIO && (
          <div className="panel" style={{marginTop:14}}>
            <div style={{fontSize:13, fontWeight:600, marginBottom:10}}>Back up or restore your progress</div>
            <div style={{display:"flex", gap:8, flexWrap:"wrap", marginBottom:12}}>
              <button className="ghost" onClick={downloadTxt}>⬇ Download .txt</button>
              <label className="ghost" style={{cursor:"pointer"}}>
                ⬆ Import .txt
                <input type="file" accept=".txt,text/plain" onChange={importFile} style={{display:"none"}} />
              </label>
            </div>
            <div style={{fontSize:11.5, color:"var(--dim)", marginBottom:6}}>Or copy/paste the text manually:</div>
            <textarea className="field" style={{height:60, fontFamily:"var(--mono)", fontSize:11, color:"var(--dim)"}} value={ioText} onChange={e=>setIoText(e.target.value)} spellCheck={false} aria-label="Progress data" />
            <div style={{display:"flex", gap:8, marginTop:8}}>
              <button className="ghost" onClick={()=>{ navigator.clipboard&&navigator.clipboard.writeText(ioText); }}>Copy</button>
              <button className="ghost" onClick={loadFromText}>Load from text</button>
              <button className="ghost" onClick={()=>setShowIO(false)}>Close</button>
            </div>
          </div>
        )}
        ...
        <div style={{display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginTop:14}}>
          <div className="searchbox">
            <span style={{fontSize:12, opacity:.55}} aria-hidden="true">🔍</span>
            <input ref={searchRef} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search steps…" aria-label="Search steps" />
            {q
              ? <>
                  <span className="mono" style={{fontSize:10.5, color:"var(--dim)", whiteSpace:"nowrap"}}>{filteredTasks.length} hit{filteredTasks.length===1?"":"s"}</span>
                  <button className="clear" onClick={()=>setSearch("")} aria-label="Clear search">✕</button>
                </>
              : <span className="kbd" aria-hidden="true">/</span>}
          </div>
          <div className="seg" role="group" aria-label="Chapter">
            <button className={chapterFilter==="all"?"on":""} onClick={()=>setChapterFilter("all")}>All</button>
            {chapters.map(ch=>(
              <button key={ch} className={chapterFilter===ch?"on":""} onClick={()=>setChapterFilter(ch)}>{ch.split(":")[0]}</button>
            ))}
          </div>
          <div className="seg" role="group" aria-label="Status">
            {[{key:"all",label:"All"},{key:"todo",label:"To do"},{key:"parallel",label:"Parallel"},{key:"postponed",label:"Later"},{key:"done",label:"Done"}].map(f=>(
              <button key={f.key} className={filter===f.key?"on":""} onClick={()=>setFilter(f.key)}>
                {f.label}{f.key!=="all"&&<span className="n">{counts[f.key]}</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="hint">
          <span><span style={{color:"var(--done)"}}>✓</span> done</span>
          <span><span style={{color:"var(--par)"}}>●</span> parallel</span>
          <span><span style={{color:"var(--later)"}}>●</span> later</span>
          <span><span style={{color:"var(--warn)"}}>▲n</span> skills behind</span>
          <span>≈ est. time</span>
          <span>timeline above jumps to any step</span>
        </div>

        {filter==="postponed" && filteredTasks.length>1 && (
          <div style={{fontSize:11.5, color:"var(--dim)", margin:"12px 0 4px", textAlign:"center"}}>↕ Drag rows to reorder how you'll come back to them</div>
        )}
        {filteredTasks.length===0 && (
          <div className="empty">
            <div style={{fontSize:34, marginBottom:12}} aria-hidden="true">{q?"🔍":"⚔️"}</div>
            <div style={{fontSize:15}}>{q?`No steps match “${search.trim()}”.`:"Nothing here with this filter."}</div>
            {q && <button className="ghost" style={{marginTop:14}} onClick={()=>setSearch("")}>Clear search</button>}
          </div>
        )}
        ...
        {filteredTasks.map(task=>{
          ...
          const est=estimate(task);
          const xpEntries=Object.entries(task.xp_gains||{}).filter(([sk,xp])=>SKILL_FULL[sk]&&xp>0);
          const behind=Object.entries(task.expected||{})
            .filter(([sk,lvl])=>SKILL_FULL[sk]&&(state.levels[sk]||1)<lvl)
            .sort((a,b)=>(b[1]-(state.levels[b[0]]||1))-(a[1]-(state.levels[a[0]]||1)));
          const behindTip=behind.map(([sk,l])=>`${SKILL_FULL[sk]} ${state.levels[sk]||1}→${l}`).join(", ");

          const sect = filter!=="postponed" && task.chapter!==lastChapter
            ? <div className="sect" key={"s-"+task.chapter}>
                <span className="nm">{task.chapter}</span>
                <span className="ct mono">{chapterMeta[task.chapter].done}/{chapterMeta[task.chapter].total}</span>
                {chapterMeta[task.chapter].eta>0 && <span className="eta">{fmtH(chapterMeta[task.chapter].eta)} left</span>}
              </div>
            : null;
          lastChapter=task.chapter;

          const rowCls=["row", task.isSub&&"sub", isDone&&"isdone", isNext&&"cur", isExpanded&&"open", task.tag&&"opt-row"].filter(Boolean).join(" ");
          return (
            <React.Fragment key={task.id}>
              {sect}
              <div className={rowCls} ref={el=>{rowRefs.current[task.id]=el;}} draggable={draggable}
                onDragStart={()=>{ if(draggable) dragId.current=task.id; }}
                onDragOver={e=>{ if(draggable) e.preventDefault(); }}
                onDrop={()=>{ if(draggable) handleDrop(task.id); }}>
                <div className="line1" style={draggable?{cursor:"grab"}:null} onClick={()=>toggleExpand(task.id)}>
                  {draggable && <span style={{color:"var(--faint)", fontSize:14, flexShrink:0}} aria-hidden="true">⠿</span>}
                  <button className={"chk"+(isDone?" on":"")} aria-label={isDone?"Mark not done":"Mark done"}
                    onClick={e=>{ e.stopPropagation(); toggle(task.id,'done'); }}>{isDone?"✓":""}</button>
                  <span className="lbl">{task.label}</span>
                  {!isDone && isParallel && <span className="st par" title="Doing in parallel" />}
                  {!isDone && !isParallel && isPostponed && <span className="st later" title="Postponed" />}
                  <span className="ttl">
                    {isNext && <span className="tag now" style={{marginRight:8}}>NOW</span>}
                    {task.activity}
                    {note && <span title="Has a note" style={{marginLeft:6, fontSize:11.5}}>📝</span>}
                  </span>
                  {task.tag==="opt-rec" && <span className="tag rec" title="Optional, but the guide recommends it">OPTIONAL·REC</span>}
                  {task.tag==="opt" && <span className="tag opt" title="Optional — skip freely">OPTIONAL</span>}
                  {hasLevels && !isDone && behind.length>0 && <span className="mchip bd" title={"Behind: "+behindTip}>▲{behind.length}</span>}
                  {est && !isDone && <span className="mchip eta" title={"Rough duration — driven by "+SKILL_FULL[est.driver]+" xp"}>{fmtH(est.hours)}</span>}
                  {task.gp_change!=null && <span className={"mchip "+(task.gp_change>=0?"plus":"minus")} title="GP change">{formatGP(task.gp_change)}</span>}
                  <div className="acts">
                    <button className={"ib"+(isParallel?" on-par":"")} title="Doing in parallel" aria-pressed={isParallel}
                      onClick={e=>{ e.stopPropagation(); toggle(task.id,'parallel'); }}>⚡</button>
                    <button className={"ib"+(isPostponed?" on-later":"")} title="Postpone for later" aria-pressed={isPostponed}
                      onClick={e=>{ e.stopPropagation(); toggle(task.id,'postponed'); }}>⏳</button>
                    <span className="ib caret" aria-hidden="true">›</span>
                  </div>
                </div>
                <div className="exp"><div>
                  <div className="detail">
                    {task.guide && <div className="guide">{task.guide}</div>}
                    {xpEntries.length>0 && (
                      <>
                        <div className="dl">XP gains{est?` · ≈${est.hours<1?Math.max(1,Math.round(est.hours*60))+" min":est.hours.toFixed(1)+" h"} via ${SKILL_FULL[est.driver]}`:""}</div>
                        <div className="chips">
                          {xpEntries.map(([sk,xp])=>(
                            <span key={sk} className="c"><span className="dot" style={{background:SKILL_COLORS[sk]}} /><b>{SKILL_FULL[sk]}</b><span className="mono">{formatXP(xp)}</span></span>
                          ))}
                        </div>
                      </>
                    )}
                    {behind.length>0 && (
                      <>
                        <div className="dl bd">Catch up first — guide expects</div>
                        <div className="chips">
                          {behind.map(([sk,lvl])=>(
                            <span key={sk} className="c bd"><span className="dot" style={{background:SKILL_COLORS[sk]}} /><b>{SKILL_FULL[sk]} {lvl}</b><span className="mono" style={{color:"var(--faint)"}}>you: {state.levels[sk]||"?"}</span></span>
                          ))}
                        </div>
                      </>
                    )}
                    {task.gp_total!=null && (
                      <>
                        <div className="dl">GP after step</div>
                        <span className="mono" style={{fontSize:13, fontWeight:600}}>{task.gp_total>=1e6?(task.gp_total/1e6).toFixed(2)+"M":(task.gp_total/1e3).toFixed(0)+"k"} gp</span>
                      </>
                    )}
                    <div className="dl">My notes</div>
                    <textarea className="field notes" value={note} onChange={e=>setNote(task.id,e.target.value)} onClick={e=>e.stopPropagation()}
                      placeholder="e.g. stopped at 60 slayer, still need 2 diaries…" />
                  </div>
                </div></div>
              </div>
            </React.Fragment>
